...
import { XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Area, Line, ComposedChart, } from 'recharts';
import { ArrowLeft, Play, Square, Loader2, CheckCircle, AlertCircle, TrendingUp, Users, Dna, Activity, Clock, Wifi, } from 'lucide-react';
...
export default function SimulationView() {
    ...
    if (isLoading || (!sim && !isError)) {
        return (_jsxs("div", { className: "flex flex-col items-center justify-center min-h-[60vh] gap-4", children: [_jsx(Loader2, { className: "w-8 h-8 text-primary animate-spin" }), loadingTooLong && (_jsxs(motion.div, { initial: { opacity: 0, y: 8 }, animate: { opacity: 1, y: 0 }, className: "flex items-center gap-2 px-4 py-2 rounded-xl bg-cyan/10 border border-cyan/20 text-cyan text-sm", children: [_jsx(Wifi, { className: "w-4 h-4" }), "Server is warming up \u2014 this takes ~15 seconds on first load"] }))] }));
    }
    // ── Error state ───────────────────────────────────────────────────────────
    if (isError || !sim) {
        return (_jsxs("div", { className: "flex flex-col items-center justify-center min-h-[60vh] gap-4 text-center", children: [_jsx(AlertCircle, { className: "w-10 h-10 text-red-400" }), _jsx("h2", { className: "text-xl font-bold", children: "Failed to load simulation" }), _jsx("p", { className: "text-dim text-sm max-w-sm", children: "The server may be unavailable or this simulation doesn't exist." }), _jsx(Link, { to: "/dashboard", className: "px-4 py-2 rounded-xl bg-surface border border-border text-sm hover:border-primary/30 transition-colors", children: "\u2190 Back to Dashboard" })] }));
    }
...
function OrganismRow({ org, rank, maxFitness, minFitness }) {
    ...
    return (_jsxs("div", { className: "flex items-start gap-3", children: [_jsx("span", { className: "text-xs text-muted w-5 font-mono text-right mt-1", children: rank }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("div", { className: "flex items-center justify-between text-xs mb-1 gap-2", children: [_jsxs("button", { onClick: () => setExpanded(v => !v), className: "font-mono text-dim text-left hover:text-cyan transition-colors truncate", children: ["[", expanded ? full : preview, !expanded && org.genome.length > 5 ? '…' : '', "]"] }), _jsx("span", { className: "text-primary font-semibold flex-shrink-0", children: org.fitness.toFixed(5) })] }), _jsx("div", { className: "h-1 bg-border rounded-full overflow-hidden", children: _jsx("div", { className: "h-full bg-gradient-to-r from-primary to-cyan rounded-full transition-all duration-300", style: { width: `${pct}%` } }) })] })] }));
}
